...
class ProductController
{
    ...
    public function processCheckout()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $address = $_POST['street'] . ', ' . $_POST['ward'] . ', ' . $_POST['district'] . ', ' . $_POST['province'];
            ...
            try {
                ...
                // Lưu từng sản phẩm trong giỏ hàng vào order_details
                $cart = $_SESSION['cart'];
                foreach ($cart as $product_id => $item) {
                    $total = $item['quantity'] * $item['price']; // Tính thủ công
              ...
              VALUES (:order_id, :product_id, :quantity, :price, :total)";
                    ...
                    $stmt->bindParam(':total', $total); // ✅ Bắt buộc
                    $stmt->execute();
                }



                // Xoá giỏ hàng sau khi hoàn tất
                unset($_SESSION['cart']);
    ...
    public function updateQuantity()
    {
        ...
        if ($action === 'increase') {
            ...
            if ($_SESSION['cart'][$id]['quantity'] <= 0) {
                unset($_SESSION['cart'][$id]); // Tự động xóa nếu về 0
            }
        }
